...
class SnakeGame:
    ...
    def do_training(self):
        ...
        for game in range(1, self.args.NUM_TRAIN_ITER + 1):
            ...
            is_dead = False
            current_action = self.agent.agent_action(current_state, 0, is_dead)
            while not is_dead:
                current_state, points, is_dead = self.env.step(current_action)
                current_action = self.agent.agent_action(current_state, points, is_dead)
        ...
        print("Training takes", time.time() - start_time, "seconds")
